# هندسة مجموعة البيانات: الخندق التنافسي الدائم

> الـ fine-tune لا يكون أفضل من مجموعة البيانات التي تقف خلفه. مجموعة البيانات هي الميزة التنافسية الفعلية.

**النوع:** بناء
**اللغات:** Python
**المتطلبات:** الدرس 09-01 (سلّم القرار)، المرحلة 02 (الاسترجاع والـ RAG)
**الوقت:** ~60 دقيقة
...
- تعريف عقد مدخلات/مخرجات دقيق قبل جمع مثال واحد
- بناء فئة DatasetPipeline تتحقق من بيانات الـ fine-tuning وتزيل المكرَّر منها وتقسّمها
- التعرّف على أنماط مجموعة البيانات الخمسة المضادة التي تنتج نموذجًا مُحسَّنًا أسوأ من الـ prompting
- تطبيق إرشادات التعليق (annotation) على مجموعة بيانات دعم عملاء واقعية
- كتابة ملف JSONL بتنسيق الـ fine-tuning الذي تتوقعه الـ APIs المُدارة

---

## المشكلة

فريق يبني مجموعة بيانات للـ fine-tuning عبر تصدير 2,000 محادثة من نظام تذاكر الدعم لديهم. ينظّفونها، ويزيلون المعلومات الشخصية (PII)، ويرفعونها، ويطلقون مهمة الـ fine-tuning، ويطلقون النتيجة. النموذج المُحسَّن أسوأ من نموذجهم الأساسي مع system prompt. ليس أسوأ قليلًا فقط - بل أسوأ بشكل ملحوظ.

ما الذي حدث: احتوت مجموعة البيانات على 400 مثال شبه مكرَّر من أسبوع مزدحم واحد، و200 مثال أعطى فيها الوكيل ردًّا صحيحًا تقنيًّا لكنه نزِق، و150 مثالًا اقتُطعت فيها إجابة الوكيل في منتصف الجملة، و300 مثال كانت في الواقع ملاحظات داخلية وليست ردودًا للعملاء. لم يراجع أحد الأمثلة قبل التدريب. فتعلّم النموذج أن يكون مختصرًا، نزِقًا قليلًا، وأن يتوقف أحيانًا في منتصف الجملة.

جودة مجموعة البيانات تتغلّب على حجمها. النموذج المُدرَّب على 200 مثال منسَّق بعناية يتفوق باستمرار على النموذج المُدرَّب على 2,000 مثال مولَّد آليًّا أو غير منسَّق. هذا مثبت جيدًا في الممارسة. انضباط التنسيق هو الخندق التنافسي.

---

## المفهوم

### خطوات هندسة مجموعة البيانات الخمس

```mermaid
flowchart LR
    A[Define contract\ninput/output spec] --> B[Collect raw examples\nproduction traffic]
...
لكل خطوة بوابة جودة. لا يمكنك تخطّي الخطوة 1 (العقد) أو الخطوة 4 (المراجعة البشرية) دون إنتاج مجموعة بيانات أقل جودة. الخطوة 4 هي الأعلى رافعةً والأكثر تخطّيًا.

### عقد المدخلات/المخرجات

يحدد العقد الشكل الدقيق لكل مثال في مجموعة بياناتك. قبل جمع مثال واحد، يجب أن تكون قادرًا على الإجابة على:

- ما هو التنسيق الدقيق للمدخل؟ (بنية الرسالة، هل الـ system prompt مُضمَّن أم لا، الحقول المتغيرة)
- ما هو التنسيق الدقيق للمخرج المطلوب؟ (مخطط JSON، نص حر بقيود، كود)
- ما الذي يجعل مخرجًا "جيدًا" مقابل "مقبول" مقابل "مرفوض"؟
- ما القواعد الصارمة التي يجب ألا يخالفها أي مخرج أبدًا؟

عقد يستغرق 30 دقيقة لكتابته يوفّر 10 ساعات من الارتباك في التعليق.

### تنسيق JSONL للـ Fine-Tuning

تنسيق الـ fine-tuning القياسي هو JSONL (كائن JSON واحد لكل سطر). كل كائن هو مصفوفة `messages` تطابق تنسيق الدردشة:
...
كل مثال يستخدم الـ system prompt نفسه. التنوع يكون في رسالة المستخدم وفي رد المساعد المثالي. يتعلّم النموذج النمط من مئات أزواج (user, ideal_response) في سياق ذلك الـ system prompt.

### أنماط مجموعة البيانات المضادة

```
ANTI-PATTERN          WHAT HAPPENS                    HOW TO CATCH IT
------------------    ----------------------------    -------------------
Near-duplicates       Model memorizes specific         Dedup by similarity
                      phrasings, poor generalization   (edit distance or embed)

Inconsistent output   Model averages between styles    Human review with rubric
format                producing a hybrid nobody wants

Auto-generated        Model learns the generator's     Review 100% of examples
without review        errors and biases

Wrong examples        Model learns incorrect behavior  Annotator agreement check
(bad responses)       confidently

Imbalanced classes    Model overweights dominant       Count by category
                      category, underperforms others   before splitting
```

### لماذا تتغلّب 200 منسَّقة على 2,000 خام

هذا نمط يُلاحَظ باستمرار في الـ fine-tuning الإنتاجي:

```
200 examples (human reviewed, consistent format, edge cases represented)
vs
2,000 examples (auto-generated, 20% duplicates, inconsistent format)

Result: the 200-example model is measurably better on held-out eval
```

السبب: الـ fine-tuning هو gradient descent على أمثلتك. كل مثال سيئ يعلّم النموذج شيئًا خاطئًا. كل شبه مكرَّر يهدر حوسبة التدريب. يتعلّم النموذج السلوك المتوسط في مجموعة بياناتك. إن أظهر 10% من مجموعة بياناتك سلوكًا سيئًا، فسيُظهِر نموذجك المُحسَّن سلوكًا سيئًا 10% من الوقت، بشكل حتمي.
...
> **اختبار من الواقع:** يراجع فريق التعليق لديك الأمثلة منذ ثلاثة أيام، وهناك خلاف بالفعل: أحد المعلّقين يصنّف إجابة من جملتين على أنها "قصيرة جدًّا وغير مفيدة" بينما يصنّفها آخر على أنها "مختصرة بشكل مناسب". كيف تحلّ خلافات التعليق قبل أن تلوّث مجموعة البيانات؟ وما المُخرَج الذي تحتاجه قبل بدء التعليق؟

---

## الاستخدام

شغّل الـ pipeline على مجموعة بيانات دعم العملاء الاصطناعية:
...
```
Validation: 48 valid, 2 rejected from 50 total
Deduplication: removed 1 duplicates, 47 unique examples remain
...
Writing JSONL to ./dataset_output:
  Wrote 37 examples to ./dataset_output/train.jsonl
  Wrote 4 examples to ./dataset_output/val.jsonl
  Wrote 6 examples to ./dataset_output/test.jsonl
```

يُرفَض مثالان: رد "6pm." (قصير جدًّا)، وتُزال نسخة شبه مكرَّرة من مثال إعادة تعيين كلمة المرور. الـ pipeline يلتقط كلتا المشكلتين تلقائيًّا.

مخرجات JSONL في `train.jsonl` جاهزة للرفع إلى API مُدارة للـ fine-tuning (انظر الدرس 03).

> **نقلة في المنظور:** يقول استشاري: "لست بحاجة إلى كتابة إرشادات تعليق - فالمعلّقون لديك أشخاص أذكياء، وسيكتشفون كيف يبدو 'الجيد' من الأمثلة." لماذا هذا خاطئ، وما الذي ينتجه غياب إرشادات التعليق في النموذج المُحسَّن تحديدًا؟

---

## التسليم

المُخرَج لهذا الدرس هو `outputs/skill-dataset-pipeline.md` - قالب قابل لإعادة الاستخدام لتعريف عقد مجموعة بيانات fine-tuning وإرشادات التعليق.

شغّل الـ pipeline لتوليد ملفات JSONL:

```bash
python main.py
# Output: ./dataset_output/{train,val,test}.jsonl
```

ملف `train.jsonl` جاهز للرفع إلى الـ API المُدارة للـ fine-tuning في الدرس 03.

---

## التقييم

**الفحص 1: معدل اتفاق التعليق.**
قبل معالجة مجموعة البيانات الكاملة، خذ 20 مثالًا عشوائيًّا واطلب من معلّقَين تقييمها بشكل مستقل باستخدام معيارك (rubric). احسب معدل الاتفاق: عدد المطابقات / 20. أقل من 80% يعني أن العقد غامض. أعِد صياغة المعايير الغامضة قبل المتابعة.

**الفحص 2: توزيع الفئات.**
عُدّ الأمثلة حسب الفئة (نوع السؤال، النية، الموضوع). إن كانت أي فئة واحدة تتجاوز 30% من مجموعة بياناتك، فسيبالغ النموذج المُحسَّن في وزنها. أضِف أمثلة في الفئات قليلة التمثيل قبل التدريب.

**الفحص 3: معدل الرفض عند التحقق.**
إن رفض الـ pipeline لديك أكثر من 20% من الأمثلة عند التحقق، فإما أن عتبات العقد خاطئة أو أن جودة بياناتك الخام منخفضة. حقّق في الأمثلة المرفوضة قبل تعديل العتبات.

**الفحص 4: معدل تصادم البصمات.**
إن أزال إزالة التكرار أكثر من 10% من الأمثلة، فإن عملية جمع بياناتك تأخذ نفس المدخلات مرارًا. أصلِح أخذ العينات قبل جمع المزيد من البيانات - فأنت تهدر جهد التعليق على أمثلة شبه متطابقة.

**الفحص 5: فحص عيّنة من جودة المخرجات.**
بعد تشغيل الـ pipeline، اقرأ يدويًّا 10 أمثلة عشوائية من مجموعة التدريب. اسأل: هل سأكون راضيًا إن رد النموذج المُحسَّن مثل هذا تمامًا، في كل مرة؟ إن كانت أي إجابة "لا"، فينبغي رفض ذلك المثال أو إعادة كتابته.
