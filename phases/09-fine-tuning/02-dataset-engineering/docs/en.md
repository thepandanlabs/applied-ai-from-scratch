...
> A fine-tune is only as good as the dataset behind it. The dataset is the actual competitive advantage.

**Type:** Build
**Languages:** Python
**Prerequisites:** Lesson 09-01 (Decision Ladder), Phase 02 (Retrieval and RAG)
**Time:** ~60 min
...
- Define a precise input/output contract before collecting a single example
- Build a DatasetPipeline class that validates, deduplicates, and splits fine-tuning data
- Identify the five dataset anti-patterns that produce a worse fine-tuned model than prompting
- Apply annotation guidelines to a realistic customer support dataset
- Write a JSONL file in the fine-tuning format expected by managed APIs
...
What went wrong: the dataset contained 400 near-duplicate examples from a single busy week, 200 examples where the agent gave a technically correct but grumpy response, 150 examples where the agent's answer was cut off mid-sentence, and 300 examples that were actually internal notes, not customer responses. Nobody reviewed the examples before training. The model learned to be concise, slightly grumpy, and to sometimes stop mid-sentence.

Dataset quality beats dataset size. A model trained on 200 carefully curated examples consistently outperforms a model trained on 2,000 auto-generated or uncurated examples. This is well established in practice. The curation discipline is the moat.
...
- What is the EXACT format of the desired output? (JSON schema, free text with constraints, code)
- What makes an output "good" vs "acceptable" vs "reject"?
- What are the hard rules an output must never violate?

A contract that takes 30 minutes to write saves 10 hours of annotation confusion.

### JSONL Format for Fine-Tuning

The standard fine-tuning format is JSONL (one JSON object per line). Each object is a `messages` array matching the chat format:
...
Every example uses the same system prompt. The variation is in the user message and the ideal assistant response. The model learns the pattern from hundreds of (user, ideal_response) pairs in the context of that system prompt.
...
### Why 200 Curated Beats 2,000 Raw

This is a consistently observed pattern in production fine-tuning:
...
The reason: fine-tuning is gradient descent on your examples. Every bad example teaches the model something wrong. Every near-duplicate wastes training compute. The model learns the average behavior in your dataset. If 10% of your dataset shows bad behavior, your fine-tuned model will exhibit bad behavior 10% of the time, deterministically.
...
> **Real-world check:** Your annotation team has been reviewing examples for three days and there is already disagreement: one annotator marks a 2-sentence answer as "too short and unhelpful" while another marks it as "appropriately concise." How do you resolve annotation disagreements before they contaminate the dataset? What artifact do you need before annotation starts?

---

## Use It

Run the pipeline on the synthetic customer support dataset:
...
Two examples are rejected: the "6pm." response (too short) and the near-duplicate password reset example is deduplicated. The pipeline catches both problems automatically.

The JSONL output in `train.jsonl` is ready to upload to a managed fine-tuning API (see Lesson 03).

> **Perspective shift:** A consultant says: "You don't need to write annotation guidelines - your annotators are smart people, they'll figure out what 'good' looks like from the examples." Why is this wrong, and what does a lack of annotation guidelines produce in the fine-tuned model specifically?

---

## Ship It

The artifact for this lesson is `outputs/skill-dataset-pipeline.md` - a reusable template for defining a fine-tuning dataset contract and annotation guidelines.

Run the pipeline to generate the JSONL files:
...
The `train.jsonl` file is ready for upload to the managed fine-tuning API in Lesson 03.

---

## Evaluate It

**Check 1: Annotation agreement rate.**
Before processing the full dataset, take 20 random examples and have two annotators rate them independently using your rubric. Compute agreement rate: matches / 20. Below 80% means the contract is ambiguous. Rewrite the ambiguous criteria before proceeding.

**Check 2: Category distribution.**
Count examples by category (question type, intent, topic). If any single category is more than 30% of your dataset, the fine-tuned model will over-index on it. Add examples in underrepresented categories before training.

**Check 3: Validation rejection rate.**
If your pipeline rejects more than 20% of examples at validation, the contract thresholds are wrong or your raw data quality is low. Investigate the rejected examples before adjusting thresholds.

**Check 4: Fingerprint collision rate.**
If deduplication removes more than 10% of examples, your data collection process is sampling the same inputs repeatedly. Fix the sampling before collecting more data - you are wasting annotation effort on near-identical examples.

**Check 5: Output quality spot check.**
After the pipeline runs, manually read 10 random examples from the training set. Ask: would I be satisfied if the fine-tuned model responded exactly like this, every time? If any answer is "no," that example should be rejected or rewritten.
