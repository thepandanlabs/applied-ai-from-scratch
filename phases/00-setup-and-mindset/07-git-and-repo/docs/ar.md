...
> تغييرات الـ prompt هي تغييرات في الكود. إذا لم تستطع عمل diff لها، فلن تستطيع عمل debug لها.
...
- عمل commit لأول سكربت Python ووضع tag لنسخة عاملة
...
خط معالجة الـ RAG لديك أصبح يُرجع إجابات أسوأ بعد آخر نشر (deploy) يوم الجمعة. تسأل المهندس الذي عدّله: ماذا فعلت؟ يقول: "عدّلت الـ prompt قليلًا فقط". تطلب أن ترى الـ diff. لا يوجد diff: عدّل نص الـ prompt مباشرةً في ملف Python وهو يعمل، وجرّبه يدويًا، وقال "هذا أفضل"، ثم نشره. أما الـ prompt السابق فقد ضاع.

الآن ليس لديك أي طريقة لمقارنة الـ prompt الحالي بالذي كان يعمل. لا تستطيع قياس ما إذا كان التغيير قد ساعد أو أضرّ بمجموعة التقييم (eval set) لديك. ولا تستطيع الرجوع للوراء. أنت تُصحّح (debug) تراجعًا دون خط أساس (baseline).

هذه ليست حالة افتراضية. إنها تحدث في كل فريق AI يتعامل مع نص الـ prompt كأنه إعدادات (configuration) بدلًا من أن يكون كودًا. الـ prompts منطق (logic). وتغييرات الـ prompt عمليات نشر. إذا لم يكن التغيير في git مع رسالة commit ذات معنى، فأنت لا تملك إدارة نسخ: لديك ملف بتاريخ من إصدار واحد.
...
مشاريع AI لها ملف مخاطر مختلف عن مشاريع البرمجيات العادية. الـ `.gitignore` الخاطئ قد يؤدي إما إلى تسريب الأسرار (secrets) أو إلى فقدان قابلية إعادة الإنتاج (reproducibility).
...
عندما تُشغّل `git log` على مستودع AI بعد ستة أشهر، يُفترض أن تستطيع إعادة بناء تاريخ الـ prompt كخط زمني للتغيّرات السلوكية.
...
> **اختبار من الواقع:** مشروع AI لدى فريقك يعمل منذ ثلاثة أشهر. يسألك زميلك: "هل نحتاج فعلًا إلى git tags للـ prompts؟ سجل الـ commit يُظهر أصلًا كل تغيير." متى تكون الـ tags مفيدةً تحديدًا لإصدار الـ prompt على نحوٍ لا توفّره لك hashes الـ commit وحدها؟
...
بمجرد أن يصبح مشروع AI لديك في git، تغطّي هذه الأوامر الأربعة 90% من احتياجاتك اليومية لإدارة النسخ:
...
> **نقلة في المنظور:** يُجادل قائد فريقك التقني بأن نص الـ prompt ينبغي أن يوجد في قاعدة بيانات أو نظام إدارة إعدادات (config management) بدلًا من git، لأن "هذا غرض الإعدادات". متى يكون هذا الرأي صحيحًا، ومتى يجعل debugging تراجعات AI أصعب فعليًا؟
...
ينشئ السكربت مستودع git كاملًا في `/tmp/ai_project_demo` ويأخذك عبر سير العمل بالكامل.
...
عدّل نص prompt في سكربتك. شغّل `git diff` قبل الـ commit وتأكّد أن الـ diff يُظهر بالضبط ما الذي تغيّر. اعمل له commit برسالة تصف القصد السلوكي للتغيير، لا التعديل الميكانيكي. سيّئة: "changed prompt". جيدة: "prompt: add chain-of-thought instruction to improve multi-step reasoning."

**الفحص 3: اعثر على الـ commit الذي غيّر السلوك.**

إذا كان لديك 5 commits على الأقل في مستودعك، استخدم `git log --oneline` لتحديد الـ commit الذي غيّرت فيه prompt أو معامل نموذج (model parameter). استخدم `git show <hash>` للتأكد من أنك تستطيع استرجاع الحالة الدقيقة للكود عند تلك النقطة. هذه هي المهارة التي ستستخدمها عند debugging تراجعات الإنتاج (production regressions).
